...
export default function PostCard({ post, currentUser, currentUserProfile, onProfileClick, onPostDeleted, onVisible }) {
  ...
  async function postComment() {
    if (!comment.trim() || !currentUser || commentLoading) return;
    ...
    try {
      ...
      if (e?.status === 422) {
        ...
        const reasonStr = (e.data?.reasons || []).join(", ") || "policy violation";
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => onVisible(post._id, entry.intersectionRatio),
